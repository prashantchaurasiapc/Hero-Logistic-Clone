  ...
  useEffect(() => {
    const fetchStockForScan = async () => {
      ...
      try {
        ...
        console.error('Failed to fetch stock for scanning:', err);
      } finally {
        ...
        <div style={S.modalOverlay} onClick={handleCloseModal}>
          <div style={S.modalContent} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={S.modalBody}>
              ...
              {stockItems.length > 0 && (
                <div style={{ marginBottom: 10 }}>
                  ...
                  >
                    ...
                    {stockItems.map(item => {
                      const codeVal = item.vin || item.rego || item.stockRef || item.id;
                      const titleStr = `${item.make || ''} ${item.model || ''}`.trim() || item.vehicleType || 'Stock Item';
                      return (
              ...
              <input
                type="text"
                placeholder="Or enter barcode/VIN manually (e.g. QLD-88A or VIN-7YV1HP82A81920)"
                value={decoderInput}
                onChange={(e) => setDecoderInput(e.target.value)}
                style={S.modalInput}
              />
