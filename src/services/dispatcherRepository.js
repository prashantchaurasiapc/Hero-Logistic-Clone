...
class DispatcherRepository {
  ...
  async syncWithBackend() {
    ...
    try {
      ...
      // 1. Fetch loads
      const loadsRes = await api.get('/loads').catch(ignoreAuthErr);
      ...
      // 2. Fetch drivers
      const driversRes = await api.get('/drivers').catch(ignoreAuthErr);
      ...
      // 3. Fetch vehicles
      const vehiclesRes = await api.get('/vehicles').catch(ignoreAuthErr);
      ...
      // 4. Fetch customers
      const customersRes = await api.get('/customers').catch(ignoreAuthErr);
