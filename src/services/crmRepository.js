...
class CRMRepository {
  ...
  async syncWithBackend() {
    // Skip Sales CRM sync when inside Driver portal or unauthenticated
    ...
    try {
      const ignoreAuthErr = (err) => {
        if (err?.response?.status === 401 || err?.response?.status === 403) return null;
        throw err;
      };

      // 1. Fetch leads
      const leadsRes = await api.get('/leads').catch(ignoreAuthErr);
      ...
      // 2. Fetch demos
      const demosRes = await api.get('/demo-bookings').catch(ignoreAuthErr);
      ...
      // 3. Fetch proposals
      const proposalsRes = await api.get('/proposals').catch(ignoreAuthErr);
      ...
      // 4. Fetch followups
      const tasksRes = await api.get('/follow-up-tasks').catch(ignoreAuthErr);
      ...
      // 5. Fetch sales reps
      const repsRes = await getSalesReps().catch(ignoreAuthErr);
