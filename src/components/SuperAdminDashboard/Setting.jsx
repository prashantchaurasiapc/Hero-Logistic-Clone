...
import { 
  Settings as SettingsIcon, ShieldCheck, Database, CheckCircle2, Save, Sparkles, Lock, Key, Globe, HelpCircle, Loader2 
} from 'lucide-react';
...
export default function Settings() {
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (res?.data?.success && res?.data?.data && isMounted) {
          ...
          if (data.defaultLanguage !== undefined) setDefaultLanguage(data.defaultLanguage);
          if (data.defaultTrialDays !== undefined) setDefaultTrialDays(data.defaultTrialDays);
          if (data.basePricePerCompany !== undefined) setBasePricePerCompany(data.basePricePerCompany);

          if (data.forceMfaAdmins !== undefined) setForceMfaAdmins(Boolean(data.forceMfaAdmins));
          if (data.forceMfaTenants !== undefined) setForceMfaTenants(Boolean(data.forceMfaTenants));
          if (data.passwordComplexity !== undefined) setPasswordComplexity(data.passwordComplexity);
          if (data.minPasswordLength !== undefined) setMinPasswordLength(data.minPasswordLength);
          if (data.maxLoginAttempts !== undefined) setMaxLoginAttempts(data.maxLoginAttempts);
          if (data.lockoutDurationMinutes !== undefined) setLockoutDurationMinutes(data.lockoutDurationMinutes);

          if (data.stripePublishableKey !== undefined) setStripePublishableKey(data.stripePublishableKey || '');
          if (data.stripeWebhookSecret !== undefined) setStripeWebhookSecret(data.stripeWebhookSecret || '');
          if (data.googleMapsApiKey !== undefined) setGoogleMapsApiKey(data.googleMapsApiKey || '');
          if (data.emailService !== undefined) setEmailService(data.emailService || 'sendgrid');
          if (data.emailApiKey !== undefined) setEmailApiKey(data.emailApiKey || '');
        }
      } catch (err) {
  ...
  const handleSaveSettings = async (sectionName) => {
    try {
      setIsSaving(true);
      const payload = {
        sectionName,
      ...
      if (res?.data?.success) {
        triggerToast(res.data.message || `SaaS configurations for "${sectionName}" updated successfully.`);
      } else {
        triggerToast(`Error: ${res?.data?.message || 'Failed to update settings'}`);
      }
    } catch (err) {
      console.error('Error saving settings:', err);
      triggerToast('Error saving settings to database server.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="flex-grow bg-[#F8FAFC] p-8 w-full font-sans flex flex-col items-center justify-center min-h-[400px]">
        <Loader2 className="w-8 h-8 text-brand-500 animate-spin mb-2" />
        <p className="text-xs font-bold text-slate-500">Loading Platform Settings...</p>
      </div>
    );
  }
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-xs border border-slate-100 w-full max-w-4xl space-y-6">
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-end">
              <button 
                ...
                className="bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-black font-extrabold text-xs px-6 py-3 rounded-xl shadow-sm transition-all flex items-center gap-1.5 cursor-pointer"
              >
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-xs border border-slate-100 w-full max-w-4xl space-y-6">
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-end">
              <button 
                ...
                className="bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-black font-extrabold text-xs px-6 py-3 rounded-xl shadow-sm transition-all flex items-center gap-1.5 cursor-pointer"
              >
                {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Save Security Policies
              </button>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-xs border border-slate-100 w-full max-w-4xl space-y-6">
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-end">
              <button 
                ...
                className="bg-brand-500 hover:bg-brand-600 disabled:opacity-50 text-black font-extrabold text-xs px-6 py-3 rounded-xl shadow-sm transition-all flex items-center gap-1.5 cursor-pointer"
              >
                {isSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Save Integration Keys
              </button>
            </div>
          </div>
        )}
