...
import { getCustomRoles, updateCustomRole } from '../../services/api';
...
/* Color sequence for badge pills */
...
/* Build merged permission state for a role.
   - If DB has no permissions saved at all (null/empty): default ALL to true (fully enabled).
   - If DB has saved permissions: build a full map, default missing actions to true,
     but use exact DB value for explicitly saved actions. */
const getMergedPerms = (roleName, dbPermissions) => {
  const modules = ROLE_MODULES_MAP[roleName] || ROLE_MODULES_MAP['Company Admin'];
  const permMap = {};

  // Check if DB has any meaningful permissions saved
  const hasDbData = dbPermissions &&
    typeof dbPermissions === 'object' &&
    Object.keys(dbPermissions).length > 0;

  modules.forEach(m => {
    permMap[m.menu] = {};
    m.actions.forEach(a => {
      if (!hasDbData) {
        // No DB data at all → default everything to TRUE (fully enabled)
        permMap[m.menu][a] = true;
      } else {
        // DB has data → check if this specific action was saved
        const dbMenuActions = dbPermissions[m.menu];
        if (dbMenuActions && typeof dbMenuActions === 'object' && a in dbMenuActions) {
          // Explicitly saved in DB → use exact DB value
          permMap[m.menu][a] = Boolean(dbMenuActions[a]);
        } else {
          // Not saved in DB for this specific action → default to TRUE
          permMap[m.menu][a] = true;
        }
      }
    });
...
export default function RolesPermissions() {
  ...
  const loadRoles = async () => {
    setIsLoading(true);
    try {
      const res = await getCustomRoles();
      const backendRoles = res.data?.data || res.data || [];

      if (Array.isArray(backendRoles) && backendRoles.length > 0) {
        const loadedRolesList = backendRoles.map(br => ({
          id: br.id,
          name: br.name,
          roleKey: br.slug || br.name.toUpperCase(),
          rawPermissions: br.permissions
        }));

        const initialPerms = {};
        loadedRolesList.forEach(r => {
          initialPerms[r.id] = getMergedPerms(r.name, r.rawPermissions);
        });

        setRoles(loadedRolesList);
        setRolesPerms(initialPerms);
      } else {
        setRoles(DEFAULT_MASTER_ROLES);
        const initialPerms = {};
        DEFAULT_MASTER_ROLES.forEach(r => {
          initialPerms[r.id] = getMergedPerms(r.name, null);
        });
        setRolesPerms(initialPerms);
      }
    } catch (err) {
  ...
  const handleOpenEditModal = (role) => {
    ...
    const existing = rolesPerms[role.id] || getMergedPerms(role.name, role.rawPermissions);
  ...
  // Save Modal Permissions to real backend database via PUT /custom-roles/:id
  const handleSaveModalPermissions = async () => {
    if (!activeModalRole) return;
    setIsSaving(true);
    try {
      const res = await updateCustomRole(activeModalRole.id, {
        permissions: modalPerms
      });

      if (res?.data?.success || res?.status === 200) {
        const savedPerms = res?.data?.data?.permissions || modalPerms;
        const merged = getMergedPerms(activeModalRole.name, savedPerms);

        setRolesPerms(prev => ({
          ...prev,
          [activeModalRole.id]: merged
        }));

        triggerToast(`Permissions updated for ${activeModalRole.name}!`);
        setActiveModalRole(null);
      } else {
        triggerToast('Failed to save permissions to server.');
      }
    } catch (err) {
      console.error('Error saving permissions:', err);
      triggerToast('Error saving permissions to database server.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const getGrantedPills = (role) => {
    const perms = rolesPerms[role.id] || getMergedPerms(role.name, role.rawPermissions);
    const pills = [];
