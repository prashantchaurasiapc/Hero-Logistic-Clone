...
import { 
  ...
  Settings, CheckCircle2, ChevronRight, Share2, Layers, RefreshCw, UploadCloud
} from 'lucide-react';
...
export default function Warehouse() {
  ...
  const handleSaveEditWarehouse = async () => {
    ...
    try {
      ...
      const payload = {
        ...editModal,
        photoUrl: editModal.photoUrl || editModal.image || editModal.photo || null,
        image: editModal.photoUrl || editModal.image || editModal.photo || null
      };
      const res = await api.put(`/company-admin/warehouse/${editModal.id}`, payload);
      if (res.data && res.data.success) {
        showToast(`Warehouse "${editModal.name}" updated!`);
        if (selectedWh && (selectedWh.id === editModal.id)) {
          setSelectedWh(prev => ({ ...prev, ...payload }));
        }
        setEditModal(null);
        await fetchWarehousesData();
      } else {
        ...
        <div className="wh-modal" onClick={e => e.stopPropagation()}>
          ...
          <div className="wh-modal-body" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
            ...
            <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 10, padding: 12, marginTop: 8 }}>
              <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#475569', marginBottom: 6 }}>Upload / Change Warehouse Photo</label>
              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                <div style={{ width: 80, height: 60, borderRadius: 8, border: '1px solid #CBD5E1', background: '#fff', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', flexShrink: 0 }}>
                  {(editModal.photoUrl || editModal.image || editModal.photo) ? (
                    <>
                      <img src={editModal.photoUrl || editModal.image || editModal.photo} alt="Preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      <button
                        type="button"
                        onClick={() => setEditModal({ ...editModal, photoUrl: '', image: '', photo: '' })}
                        style={{ position: 'absolute', top: 2, right: 2, width: 16, height: 16, borderRadius: '50%', background: '#EF4444', color: '#fff', border: 'none', fontWeight: 800, cursor: 'pointer', fontSize: 9, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                      >
                        ×
                      </button>
                    </>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (file) {
                        ...
                        reader.onloadend = () => {
                          setEditModal(prev => ({ ...prev, photoUrl: reader.result, image: reader.result, photo: reader.result }));
                        };
                    ...
                    style={{ fontSize: 11 }}
                  />
                  <span style={{ fontSize: 9, color: '#94A3B8', display: 'block', marginTop: 2 }}>Max 5MB. Updates across List & Details View.</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="wh-detail-profile">
            <div className="wh-profile-top">
              <div className="wh-profile-img">
                {(wh.photoUrl || wh.image || wh.photo) && !(wh.photoUrl || wh.image || wh.photo).includes('unsplash') ? (
                  <img src={wh.photoUrl || wh.image || wh.photo} alt={wh.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                  <div style={{ width: '100%', height: '100%', background: '#F3E8FF', color: '#7E22CE', display: 'flex', flexDirection: 'column', alignItems: 'center', justify: 'center' }}>
                    <Building size={32} />
                    <span style={{ fontSize: 9, fontWeight: 800, marginTop: 4 }}>{wh.name}</span>
                  </div>
                )}
              </div>
      ...
      <>
        ...
        <form onSubmit={handleSaveNewWarehouse} className="wh-add-page">
          ...
          <div className="wh-add-body">
            ...
            {/* 1.5. Warehouse Photo Upload */}
            <div className="wh-form-section">
              <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
                <div style={{ width: 40, height: 40, borderRadius: 10, background: '#F3E8FF', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: '#7E22CE' }}>
                  <UploadCloud size={20} />
                </div>
                <div>
                  <h2 style={{ fontSize: 13, fontWeight: 800, color: '#1E293B', margin: '0 0 2px 0' }}>WAREHOUSE PHOTO UPLOAD</h2>
                  <div style={{ fontSize: 10, fontWeight: 600, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Facility Header & List Photo</div>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ width: 140, height: 100, borderRadius: 12, border: '2px dashed #CBD5E1', background: '#F8FAFC', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative', flexShrink: 0 }}>
                  {(addForm.photoUrl || addForm.image) ? (
                    <>
                      <img src={addForm.photoUrl || addForm.image} alt="Warehouse Preview" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      <button
                        type="button"
                        onClick={() => setAddForm(prev => ({ ...prev, photoUrl: '', image: '' }))}
                        style={{ position: 'absolute', top: 4, right: 4, width: 20, height: 20, borderRadius: '50%', background: '#EF4444', color: '#fff', border: 'none', fontWeight: 800, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10 }}
                      >
                        ×
                      </button>
                    </>
                    ...
                    <div style={{ textAlign: 'center', padding: 8 }}>
                      <UploadCloud size={20} style={{ color: '#94A3B8', marginBottom: 2 }} />
                      <span style={{ fontSize: 9, fontWeight: 700, color: '#94A3B8', display: 'block' }}>No Image Picked</span>
                    </div>
                  )}
                </div>
                <div style={{ flex: 1 }}>
                  <label className="wh-label">Upload Warehouse Image File</label>
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => {
                      ...
                      if (file) {
                        ...
                        const reader = new FileReader();
                        reader.onloadend = () => {
                          setAddForm(prev => ({ ...prev, photoUrl: reader.result, image: reader.result }));
                        };
                        reader.readAsDataURL(file);
                      }
                    }}
                    style={{ fontSize: 12 }}
                  />
                  <span style={{ fontSize: 10, color: '#94A3B8', display: 'block', marginTop: 4 }}>Uploaded photo displays across Warehouses List & Warehouse Details View.</span>
                </div>
              </div>
            </div>
    ...
    <>
      ...
      <div className="wh-page">
        ...
        <div className="wh-metrics" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
          <MetricCard icon={<BoxIcon color="#8B5CF6" />} bg="#F5F3FF" label="TOTAL WAREHOUSES" value={(kpiStats?.totalWarehouses ?? 0).toString()} sub="Active Warehouses" />
          <MetricCard icon={<CheckCircleIcon color="#10B981" />} bg="#F0FDF4" label="TOTAL INVENTORY VALUE" value={kpiStats?.totalInventoryValue || '$0.00'} sub="Across all warehouses" />
          <MetricCard icon={<BoxIcon color="#F59E0B" />} bg="#FFFBEB" label="TOTAL STOCK ITEMS" value={(kpiStats?.totalStockItems ?? 0).toLocaleString()} sub="All warehouses" />
          <MetricCard icon={<ClockIcon color="#3B82F6" />} bg="#EFF6FF" label="PENDING PICK TASKS" value={(kpiStats?.pendingTasks ?? 0).toString()} sub="Requires attention" />
          <MetricCard icon={<TruckIcon color="#8B5CF6" />} bg="#F5F3FF" label="INCOMING SHIPMENTS" value={(kpiStats?.incomingShipments ?? 0).toString()} sub="In transit / Expected" />
          <MetricCard icon={<TruckIcon color="#EF4444" />} bg="#FEF2F2" label="OUTGOING SHIPMENTS" value={(kpiStats?.outgoingShipments ?? 0).toString()} sub="Scheduled / In progress" />
        </div>
                      ...
                      <tr key={i} style={{ borderBottom: '1px solid #F8FAFC' }}
                        ...
                        <td style={{ padding: '14px 20px', verticalAlign: 'middle', whiteSpace: 'nowrap' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            {(w.photoUrl || w.image || w.photo) && !(w.photoUrl || w.image || w.photo).includes('unsplash') ? (
                              <img src={w.photoUrl || w.image || w.photo} alt={w.name} style={{ width: 34, height: 34, borderRadius: 8, objectFit: 'cover', flexShrink: 0, border: '1px solid #E2E8F0' }} />
                            ) : (
                              <div style={{ width: 34, height: 34, borderRadius: 8, background: '#F3E8FF', color: '#7E22CE', border: '1px solid #E9D5FF', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                <Building size={16} />
                              </div>
                            )}
                            <div>
