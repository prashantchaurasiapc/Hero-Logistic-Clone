...
, File as FileIcon, ChevronLeft, MoreVertical, Zap, Settings, ShieldAlert, Upload, UploadCloud, Code , Printer } from 'lucide-react';
...
export default function AssetDetails({ assetData, onBack }) {
  ...
  const getSafeStr = (val, fallback = '-') => {
    ...
    if (typeof val === 'string') return val;
    if (typeof val === 'object') return val.name || val.location || val.assignedTo || fallback;
    return String(val);
  };
  ...
  const handleSaveEditAsset = async (e) => {
    ...
    try {
      const payload = {
        ...editFormData,
        photoUrl: editFormData.photoUrl || editFormData.image || null,
        image: editFormData.photoUrl || editFormData.image || null
      };
      const res = await api.put(`/company-admin/assets/${targetId}`, payload).catch(() => api.put(`/assets/${targetId}`, payload));
      const updated = res?.data?.data || editFormData;
      setAsset(prev => ({
        ...prev,
        ...editFormData,
        ...updated,
      ...
      setIsEditing(false);
      triggerToast('Asset Updated Successfully', `Asset ${editFormData.name || asset.id} details saved to database.`);
      fetchAssetDetail();
    } catch (err) {
      ...
      setAsset(prev => ({
        ...prev,
        ...editFormData,
        photoUrl: editFormData.photoUrl || editFormData.image,
        image: editFormData.photoUrl || editFormData.image
      }));
      setIsEditing(false);
      triggerToast('Asset Updated', `Asset ${editFormData.name || asset.id} details saved.`);
    }
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h1 className="text-lg sm:text-xl md:text-2xl font-black text-slate-900 tracking-tight flex flex-wrap items-center gap-2">
            {activeTab === 'Assignments & History' ? (
              <>Asset Assignments & History – {asset.name || 'Asset'} ({asset.assetTag || asset.id})</>
            ) : activeTab === 'Costs & Depreciation' ? (
              <>Asset Costs & Depreciation – {asset.name || 'Asset'} ({asset.assetTag || asset.id})</>
            ) : (
              <>Asset Details – {asset.name || 'Asset'} ({asset.assetTag || asset.id})</>
            )}
      ...
      <div className="flex flex-col xl:flex-row gap-6 items-stretch mb-6">
        ...
        <div className="flex-1 bg-white rounded-2xl border border-slate-200 p-5 shadow-sm flex flex-col md:flex-row gap-6 items-start">
          ...
          <div className="w-full md:w-[220px] shrink-0">
            {asset.image && !asset.image.includes('unsplash') ? (
              <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-900/5 h-44 md:h-48 flex items-center justify-center">
                <img
                  src={asset.image}
                  alt={asset.name}
              ...
              <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-gradient-to-br from-slate-50 to-purple-50/40 h-44 md:h-48 flex flex-col items-center justify-center p-4 text-center">
                <div className="w-14 h-14 rounded-2xl bg-purple-100 text-purple-600 border border-purple-200 flex items-center justify-center mb-2 shadow-xs">
                  <Package size={28} />
                </div>
                <span className="text-xs font-black text-slate-800 tracking-tight line-clamp-1">{asset.name || 'Asset'}</span>
                <span className="text-[10px] font-bold text-slate-400 mt-0.5 uppercase tracking-widest">{asset.category || 'Asset'}</span>
                <span className="absolute top-2 left-2 bg-emerald-50 text-emerald-600 border border-emerald-200 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider shadow-sm">
                  {asset.status}
                </span>
              </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-5 gap-x-6">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-0.5">Branch / Location</span>
                    <span className="text-xs font-bold text-slate-900">{getBranchName(asset.branch)}</span>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 bg-slate-50/60 p-4 rounded-xl border border-slate-100 text-xs">
                      ...
                      <div>
                        ...
                        <span className="font-bold text-slate-900 block">{getBranchName(asset.branch)}</span>
                      </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 text-xs space-y-2">
                    <div className="flex justify-between border-b border-slate-200/60 pb-2">
                      ...
                      <span className="font-semibold text-slate-900">{getBranchName(asset.branch)} - {getSafeStr(asset.currentLocation)}</span>
                    </div>
                    <div className="flex justify-between border-b border-slate-200/60 pb-2">
                  ...
                  <table className="w-full text-left border-collapse min-w-[800px]">
                    ...
                    <tbody>
                      <tr className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                        <td className="px-4 py-3 text-[11px] font-bold text-slate-800 whitespace-nowrap">{getBranchName(asset.branch)}</td>
        ...
        <div className="fixed inset-0 z-[99999] bg-slate-900/60 backdrop-blur-sm flex items-start justify-center pt-16 pb-6 px-4 overflow-y-auto">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-3xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[82vh]">
            ...
            <form onSubmit={handleSaveEditAsset} className="p-6 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
              ...
              {/* Section 1.5: Asset Photo Upload / Change Photo */}
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
                <h4 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
                  <UploadCloud size={14} className="text-purple-600" /> Asset Photo Upload
                </h4>
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  <div className="w-24 h-24 rounded-xl border border-slate-200 bg-white flex flex-col items-center justify-center relative overflow-hidden shrink-0 shadow-xs">
                    {(editFormData.photoUrl || editFormData.image) ? (
                      <>
                        <img src={editFormData.photoUrl || editFormData.image} alt="Asset Preview" className="w-full h-full object-cover" />
                        <button
                          type="button"
                          onClick={() => setEditFormData(prev => ({ ...prev, photoUrl: '', image: '' }))}
                          className="absolute top-1 right-1 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center text-xs font-bold shadow-md hover:bg-red-600 cursor-pointer"
                        >
                          ×
                        </button>
                      </>
                  ...
                  <div className="flex-1 w-full space-y-2">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Upload / Change Asset Photo</label>
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => {
                        ...
                        if (file) {
                          ...
                          reader.onloadend = () => {
                            setEditFormData(prev => ({ ...prev, photoUrl: reader.result, image: reader.result }));
                          };
                          reader.readAsDataURL(file);
                        }
                      }}
                      className="w-full text-xs text-slate-500 file:mr-3 file:py-2 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100 cursor-pointer"
                    />
                    <p className="text-[9px] font-semibold text-slate-400">Upload new photo to update across Table List & Details View.</p>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col">
            ...
            <div className="p-6 text-center space-y-4">
              <div className="border-2 border-dashed border-slate-300 rounded-2xl p-6 bg-slate-50 max-w-xs mx-auto space-y-3 shadow-inner">
                ...
                <div className="text-[10px] text-slate-500 font-semibold">{getBranchName(asset.branch)}</div>
              </div>
