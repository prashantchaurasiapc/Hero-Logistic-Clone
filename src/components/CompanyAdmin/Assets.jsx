...
import { 
  ...
  QrCode, MoreHorizontal, Eye, Edit, Trash2, Loader2, Upload, UploadCloud, Image as ImageIcon
} from 'lucide-react';
...
export default function Assets() {
  ...
  const handleSaveEditAsset = async () => {
    ...
    try {
      const payload = {
        ...editAssetModal,
        photoUrl: editAssetModal.photoUrl || editAssetModal.image || editAssetModal.photo || null,
        image: editAssetModal.photoUrl || editAssetModal.image || editAssetModal.photo || null
      };
      await api.put(`/company-admin/assets/${editAssetModal.realId || editAssetModal.id}`, payload);
      setAssetList(prev => prev.map(a => (a.id === editAssetModal.id || a.realId === editAssetModal.realId) ? { ...a, ...payload } : a));
                  ...
                  <tbody className="divide-y divide-slate-100 bg-white">
                    {filteredAssets.map((asset) => (
                      <tr key={asset.id} className="hover:bg-slate-50/50 transition-colors group">
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-2.5 cursor-pointer" onClick={() => setSelectedAsset(asset)}>
                            {(asset.photoUrl || asset.image || asset.photo) && !(asset.photoUrl || asset.image || asset.photo).includes('unsplash') ? (
                              <img src={asset.photoUrl || asset.image || asset.photo} alt={asset.name} className="w-9 h-9 rounded-lg object-cover border border-slate-200 shadow-xs shrink-0" />
                            ) : (
                              <div className="w-9 h-9 rounded-lg border border-slate-200 bg-purple-50 text-purple-600 flex items-center justify-center shrink-0 shadow-xs">
                                <Package size={16} />
                              </div>
                            )}
                            <div className="flex flex-col">
                              <span className="text-[10px] font-black text-blue-600 font-mono tracking-wider hover:underline">{asset.assetId || asset.id}</span>
                            </div>
                          </div>
                        </td>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-lg my-auto overflow-hidden animate-in fade-in zoom-in duration-150 max-h-[88vh] flex flex-col">
            ...
            <div className="p-6 space-y-4 overflow-y-auto custom-scrollbar flex-1 text-xs">
              ...
              {/* ASSET PHOTO UPLOAD / CHANGE PHOTO */}
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-2.5">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3">
                  <div className="w-20 h-20 rounded-xl border border-slate-200 bg-white flex flex-col items-center justify-center relative overflow-hidden shrink-0 shadow-xs">
                    {(editAssetModal.photoUrl || editAssetModal.image || editAssetModal.photo) ? (
                      <>
                        <img src={editAssetModal.photoUrl || editAssetModal.image || editAssetModal.photo} alt="Asset Preview" className="w-full h-full object-cover" />
                        <button
                          type="button"
                          onClick={() => setEditAssetModal(prev => ({ ...prev, photoUrl: '', image: '', photo: '' }))}
                      ...
                      <div className="flex flex-col items-center justify-center p-1.5 text-center">
                        <UploadCloud size={18} className="text-slate-400 mb-0.5" />
                  ...
                  <div className="flex-1 w-full space-y-1.5">
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest block">Upload / Change Photo</label>
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => {
                        ...
                        if (file) {
                          ...
                          const reader = new FileReader();
                          reader.onloadend = () => {
                            setEditAssetModal(prev => ({ ...prev, photoUrl: reader.result, image: reader.result, photo: reader.result }));
                          };
                          reader.readAsDataURL(file);
                        }
                      }}
                      className="w-full text-xs text-slate-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-purple-50 file:text-purple-700 hover:file:bg-purple-100 cursor-pointer"
                    />
                    <p className="text-[9px] font-semibold text-slate-400">Max size 5MB. Photo updates in Table & Details View.</p>
                  </div>
                </div>
              </div>
            </div>
