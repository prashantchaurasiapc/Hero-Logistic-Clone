...
const BarcodeGraphic = ({ value, width = 200, height = 44 }) => {
  const text = (value || 'LOC-A1204').toUpperCase();
  const bars = [];
  let x = 10;
  for (let i = 0; i < text.length; i++) {
    ...
    bars.push(<rect key={`b1-${i}`} x={x} y="0" width={w1 * 1.6} height={height - 12} fill="#0F172A" />);
    x += w1 * 1.6 + 2;
    bars.push(<rect key={`b2-${i}`} x={x} y="0" width={w2 * 1.6} height={height - 12} fill="#0F172A" />);
    x += w2 * 1.6 + 3;
  }
  const svgWidth = Math.max(x + 10, width);
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', background: '#fff', padding: '6px 12px', borderRadius: 8, border: '1px solid #CBD5E1' }}>
      <svg width={svgWidth} height={height} viewBox={`0 0 ${svgWidth} ${height}`}>
        <rect x="0" y="0" width="100%" height={height} fill="#fff" />
        {bars}
      </svg>
      <span style={{ fontSize: 10, fontWeight: 900, fontFamily: 'monospace', color: '#0F172A', letterSpacing: '2px', marginTop: 2 }}>*{text}*</span>
    </div>
  );
};

const QRCodeGraphic = ({ value, size = 56 }) => {
  const text = value || 'LOC-A1204';
  const grid = 7;
  const cells = [];
  for (let r = 0; r < grid; r++) {
    for (let c = 0; c < grid; c++) {
      const isCorner = (r < 2 && c < 2) || (r < 2 && c >= grid - 2) || (r >= grid - 2 && c < 2);
      const isFilled = isCorner || ((r * grid + c + text.charCodeAt((r + c) % text.length)) % 2 === 0);
      if (isFilled) {
        cells.push(
          <rect
            key={`${r}-${c}`}
  ...
  return (
    <div style={{ background: '#fff', padding: 5, borderRadius: 8, border: '1px solid #CBD5E1', display: 'inline-block' }}>
    ...
    try {
      ...
      if (res.data && res.data.success && res.data.data) {
        ...
        const mappedLocs = locs.map(l => ({
          id: l.id,
          code: l.code || (l.name ? (l.name.startsWith('LOC-') ? l.name : `LOC-${l.name.slice(0, 8).toUpperCase()}`) : 'LOC-001'),
          name: l.name || l.code || 'Location Bin',
          area: l.area || 'Standard Storage',
          type: l.type || 'Floor',
          bins: l.bins || 6,
          cap: l.cap || 100,
          used: l.used || 0,
        ...
        setLocationsList(mappedLocs);

        if (stats) {
          setKpiStats(stats);
        } else {
          const totalLocs = mappedLocs.length;
          const totalBins = totalLocs * 6;
          const totalCap = totalLocs * 100;
          const totalUsed = mappedLocs.reduce((acc, curr) => acc + (parseInt(curr.used) || 0), 0);
    ...
    <div className="wh-bins-container" style={{ background: '#F8FAFC', minHeight: '100vh', padding: '24px 32px', fontFamily: "'Inter','Outfit',sans-serif", overflowX: 'hidden' }}>
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              ...
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  <label style={{ fontSize: 12, fontWeight: 700, color: '#475569' }}>Barcode & QR Code Generation</label>
                ...
                {autoGenerateBarcode ? (
                  <div style={{ padding: '12px', background: '#F8FAFC', border: '1px solid #C7D2FE', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                    <BarcodeGraphic value={addForm.name || addForm.code || 'A-12-04'} width={180} height={42} />
                    <QRCodeGraphic value={addForm.name || addForm.code || 'A-12-04'} size={52} />
                  </div>
                ) : (
                  <div style={{ padding: '12px', background: '#F8FAFC', border: '1px dashed #CBD5E1', borderRadius: 8, fontSize: 12, color: '#94A3B8', textAlign: 'center' }}>
                    Auto barcode generation disabled
                  </div>
                )}
              </div>
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ background: '#fff', width: '560px', borderRadius: 20, padding: '28px', position: 'relative', zIndex: 1, boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', border: '1px solid #E2E8F0' }}>
            ...
            <div style={{ background: '#F8FAFC', padding: '14px', borderRadius: 12, border: '1px solid #E2E8F0', marginBottom: 20 }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: '#0F172A', textTransform: 'uppercase', marginBottom: 8, letterSpacing: '0.5px' }}>Location Barcode & QR Identifier</div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                <BarcodeGraphic value={viewLocationModal.code || viewLocationModal.name} width={200} height={44} />
                <QRCodeGraphic value={viewLocationModal.code || viewLocationModal.name} size={54} />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 16, borderTop: '1px solid #F1F5F9' }}>
              <button
                onClick={() => { const loc = viewLocationModal; setViewLocationModal(null); setPrintModalData(loc); }}
      ...
      {printModalData && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ position: 'absolute', inset: 0, background: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)' }} onClick={() => setPrintModalData(null)} />
          <div style={{ background: '#fff', width: '480px', borderRadius: 20, padding: '28px', position: 'relative', zIndex: 1, boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.3)', border: '1px solid #E2E8F0' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 12, borderBottom: '1px solid #F1F5F9' }}>
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 900, color: '#0F172A', display: 'flex', alignItems: 'center', gap: 8 }}>
                🏷️ Location Barcode Label
              </h2>
              <button onClick={() => setPrintModalData(null)} style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: '#64748B' }}>✕</button>
            </div>

            {/* Printable Label Card */}
            <div id="printable-barcode-label" style={{ background: '#fff', border: '2px solid #0F172A', borderRadius: 14, padding: '20px', textAlign: 'center', marginBottom: 24, boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
              <div style={{ fontSize: 10, fontWeight: 800, color: '#64748B', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 4 }}>
                {wh?.name || 'HERO LOGISTICS WAREHOUSE'}
              </div>
              <div style={{ fontSize: 24, fontWeight: 900, color: '#0F172A', letterSpacing: '-0.5px', marginBottom: 2 }}>
                {printModalData.code || printModalData.name}
              </div>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#475569', marginBottom: 16 }}>
                AREA: {printModalData.area || 'STORAGE'} • TYPE: {printModalData.type || 'BIN'}
              </div>

              <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 16, marginBottom: 12, flexWrap: 'wrap' }}>
                <BarcodeGraphic value={printModalData.code || printModalData.name} width={220} height={50} />
                <QRCodeGraphic value={printModalData.code || printModalData.name} size={64} />
              </div>

              <div style={{ fontSize: 9, color: '#94A3B8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px' }}>
                SCANNABLE WAREHOUSE LOCATION IDENTIFIER
              </div>
            </div>

            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              <button onClick={() => setPrintModalData(null)} style={{ padding: '10px 18px', borderRadius: 8, fontSize: 13, fontWeight: 600, border: '1px solid #E2E8F0', background: '#fff', color: '#475569', cursor: 'pointer' }}>Close</button>
              <button
                onClick={() => {
                  window.print();
                  showToast(`Printed Label for ${printModalData.code || printModalData.name}`);
                }}
                style={{ padding: '10px 22px', borderRadius: 8, fontSize: 13, fontWeight: 700, border: 'none', background: '#4F46E5', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}
              >
                🖨️ Print Label
              </button>
            </div>
