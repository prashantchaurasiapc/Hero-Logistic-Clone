...
import {
  Plus, Search, X, Star, MoreVertical, ChevronDown, Loader2,
                    ...
                    <button 
                      onClick={async () => {
                        ...
                        if (window.confirm(`Are you sure you want to cancel Load ${currentLoad.id}?`)) {
                          try {
                            const loadIdToUse = currentLoad?.rawId || currentLoad?.id;
                            await api.put(`/company-admin/loads/${loadIdToUse}`, { status: 'CANCELLED', cancellationReason: 'Cancelled by Admin' });
                          } catch (err) { /* ignore */ }
...
export default function Loads() {
  ...
  const [editModalLoad, setEditModalLoad] = useState(null);
  const [editForm, setEditForm] = useState({});
  const [isSavingEdit, setIsSavingEdit] = useState(false);

  const [cancelModalLoad, setCancelModalLoad] = useState(null);
  ...
  const openCancelModal = (load) => {
    setCancelModalLoad(load);
    setCancelReason('Customer Cancellation Request');
    setCancelNotes('');
  };

  const handleConfirmCancelLoad = async () => {
    if (!cancelModalLoad) return;
    setIsCancellingLoad(true);
    const finalReason = cancelReason === 'Other' ? (cancelNotes.trim() || 'Cancelled by Admin') : cancelReason;
    try {
      const loadId = cancelModalLoad.rawId || cancelModalLoad.id;
      await api.put(`/company-admin/loads/${loadId}`, {
        status: 'CANCELLED',
        cancellationReason: finalReason,
        dispatchNotes: cancelNotes ? `Cancellation notes: ${cancelNotes}` : undefined
      });
    } catch (err) {
      console.error('Cancel load API error:', err);
    }
    setLoadsList(prev => prev.map(l =>
      (l.id === cancelModalLoad.id || l.rawId === cancelModalLoad.rawId)
        ? { ...l, status: 'CANCELLED', statusSub: finalReason }
        : l
    ));
    triggerToast(`Load ${cancelModalLoad.id} cancelled.`, 'error');
    setCancelModalLoad(null);
  ...
  const handleSaveEdit = async () => {
    if (!editModalLoad) return;
    setIsSavingEdit(true);
    try {
      const loadId = editModalLoad.rawId || editModalLoad.id;
      await api.put(`/company-admin/loads/${loadId}`, {
        type: editForm.type,
        priority: editForm.priority,
        status: editForm.status === 'ACTIVE' ? 'IN_TRANSIT' : editForm.status,
        notes: editForm.notes
      });
      setLoadsList(prev => prev.map(l =>
        l.id === editModalLoad.id
          ? { ...l, type: editForm.type, priority: editForm.priority, status: editForm.status, notes: editForm.notes }
          : l
      ));
      triggerToast(`Load ${editModalLoad.id} updated successfully!`, 'success');
      setEditModalLoad(null);
    } catch (err) {
      console.error('Edit load error:', err);
      triggerToast('Failed to update load. Please try again.', 'error');
    } finally {
      ...
      if (res.data && res.data.success && Array.isArray(res.data.data)) {
        ...
        const mapped = res.data.data.map(item => {
          ...
          return {
            ...
            statusSub: displayStatus === 'CANCELLED' ? (item.cancellationReason || 'Cancelled') : displayStatus === 'ACTIVE' ? 'En Route' : displayStatus === 'PLANNED' ? 'Ready' : displayStatus === 'COMPLETED' ? 'Delivered' : (item.dispatchNotes || 'Draft'),
            type: item.type || 'General Freight',
            ...
            stopsCount: Array.isArray(item.stops) ? item.stops.length : (typeof item.stops === 'number' ? item.stops : 0),
    ...
    <div className="flex-grow bg-[#F8FAFC] w-full font-sans overflow-hidden flex flex-col min-h-0">
      ...
      {toast && (
        <div className={`fixed bottom-5 right-5 z-[999999] flex items-center gap-2.5 px-4 py-3 rounded-xl shadow-2xl text-xs font-bold text-white
          ${toast.type === 'error' ? 'bg-rose-600' : toast.type === 'warning' ? 'bg-amber-500' : toast.type === 'info' ? 'bg-blue-600' : 'bg-slate-900'}`}>
          <Check className="w-4 h-4 shrink-0" /> {toast.msg}
        </div>
      )}

      {/* ════ EDIT LOAD MODAL ════════════════════════════════ */}
      {editModalLoad && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[999998] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            ...
            <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-black text-slate-900 tracking-tight">Edit Load Details</h2>
                  <p className="text-[11px] font-semibold text-indigo-500 mt-0.5">Update configuration for Load {editModalLoad.id}</p>
                </div>
              </div>
              <button
                onClick={() => setEditModalLoad(null)}
            ...
            <div className="px-6 py-5 space-y-4">
              {/* Booking Customer */}
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Booking Customer</label>
                <input
                  type="text"
                  value={editForm.customer}
                  onChange={e => setEditForm(f => ({ ...f, customer: e.target.value }))}
                  className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-[13px] font-semibold text-slate-800 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all bg-white"
                  placeholder="Customer name..."
                />
              </div>

              {/* Load Type + Priority row */}
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Load Type</label>
                  <select
                    value={editForm.type}
                    onChange={e => setEditForm(f => ({ ...f, type: e.target.value }))}
                    className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-[13px] font-semibold text-slate-800 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all bg-white cursor-pointer"
                  >
                    <option>General Freight</option>
                    <option>Car Carrying</option>
              ...
              {/* Load Notes */}
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Load Notes / Special Instructions</label>
                <textarea
                  value={editForm.notes}
                  onChange={e => setEditForm(f => ({ ...f, notes: e.target.value }))}
                  rows={3}
                  className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-[13px] font-semibold text-slate-800 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 transition-all bg-white resize-none"
            ...
            <div className="px-6 py-4 bg-slate-50/70 border-t border-slate-100 flex items-center justify-end gap-3">
              <button
                onClick={() => setEditModalLoad(null)}
                className="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 font-bold text-xs rounded-xl transition-all cursor-pointer shadow-xs"
              >
                Cancel
              </button>
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs rounded-xl shadow-sm transition-all cursor-pointer disabled:opacity-50"
              >
                {isSavingEdit ? (
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
            ...
            <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-black text-slate-900 tracking-tight">Cancel Load {cancelModalLoad.id}</h2>
                  <p className="text-[11px] font-semibold text-rose-500 mt-0.5">Please confirm cancellation details below</p>
                </div>
              </div>
              <button
                onClick={() => setCancelModalLoad(null)}
                className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-slate-600 transition-colors cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="p-3.5 bg-rose-50/80 border border-rose-200/80 rounded-xl flex items-start gap-3">
                ...
                <p className="text-xs font-semibold text-rose-900 leading-normal">
                  Cancelling this load will stop dispatch notifications, update fleet tracking, and mark the status as <strong>CANCELLED</strong> in all analytics tables.
                </p>
              </div>

              {/* Cancellation Reason Selection */}
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Reason for Cancellation</label>
                <select
                  value={cancelReason}
                  onChange={e => setCancelReason(e.target.value)}
                  className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-[13px] font-bold text-slate-800 focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-100 transition-all bg-white"
                >
                  <option value="Customer Cancellation Request">Customer Cancellation Request</option>
                  <option value="Driver or Vehicle Unavailable">Driver or Vehicle Unavailable</option>
                  <option value="Route Delay / Logistics Conflict">Route Delay / Logistics Conflict</option>
              ...
              {/* Optional Notes */}
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Cancellation Notes (Optional)</label>
                <textarea
                  value={cancelNotes}
                  onChange={e => setCancelNotes(e.target.value)}
                  rows={3}
                  className="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-[13px] font-semibold text-slate-800 focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-100 transition-all bg-white resize-none"
            ...
            <div className="px-6 py-4 bg-slate-50/70 border-t border-slate-100 flex items-center justify-end gap-3">
              ...
              <button
                ...
                disabled={isCancellingLoad}
                className="flex items-center gap-2 px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-white font-black text-xs rounded-xl shadow-sm transition-all cursor-pointer disabled:opacity-50"
              >
                {isCancellingLoad ? (
                  <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Cancelling...</>
                ) : (
                  <><X className="w-3.5 h-3.5" strokeWidth={2.5} /> Confirm Cancel Load</>
                )}
              </button>
            </div>
          </div>
                  ...
                  {paged.map((load, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* ACTIONS */}
                        <td className="px-3 py-4 min-w-[140px] whitespace-nowrap" onClick={e => e.stopPropagation()}>
                          <div className="flex items-center gap-1 relative">
                            <button
                              onClick={(e) => { e.stopPropagation(); setSelectedLoad(load); }}
                              className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 border border-indigo-100 hover:bg-indigo-100 rounded-lg transition-colors cursor-pointer"
                            >
                            ...
                            {/* 3-dot kebab button */}
                            <button
                              onClick={(e) => { e.stopPropagation(); setOpenMenu(openMenu === load.id ? null : load.id); }}
                              className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer relative"
                            >
                              <MoreVertical className="w-4 h-4" />
                            </button>
                            ...
                            {openMenu === load.id && (
                              <>
                                ...
                                <div className="absolute right-0 top-8 w-52 bg-white border border-slate-200 rounded-xl shadow-xl z-50 py-1 overflow-hidden divide-y divide-slate-100"
                                  style={{ animation: 'fadeIn 0.1s ease' }}
                                >
                                  {/* View */}
                                  <div className="py-1">
                                    <button
                                      onClick={(e) => { e.stopPropagation(); setOpenMenu(null); setSelectedLoad(load); }}
                                      className="w-full px-4 py-2 text-left text-[12px] font-semibold text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 flex items-center gap-2.5 cursor-pointer transition-colors"
                                    >
                                      <Eye className="w-3.5 h-3.5 text-slate-400" /> View Load Details
                                    </button>
                                    <button
                                      onClick={(e) => { e.stopPropagation(); setOpenMenu(null); openEditModal(load); }}
                                      className="w-full px-4 py-2 text-left text-[12px] font-semibold text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 flex items-center gap-2.5 cursor-pointer transition-colors"
                                    >
                                      <Edit3 className="w-3.5 h-3.5 text-slate-400" /> Edit Load
                                    </button>
                                  ...
                                  <div className="py-1">
                                    <button
                                      onClick={(e) => {
                                        ...
                                        triggerToast(`Load ${load.id} duplicated successfully!`, 'success');
                                      }}
                                      className="w-full px-4 py-2 text-left text-[12px] font-semibold text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 flex items-center gap-2.5 cursor-pointer transition-colors"
                                    >
                                      <Layers className="w-3.5 h-3.5 text-slate-400" /> Duplicate Load
                                    </button>
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation(); setOpenMenu(null);
                                        triggerToast(`Load ${load.id} downloading manifest...`, 'info');
                                      }}
                                      className="w-full px-4 py-2 text-left text-[12px] font-semibold text-slate-700 hover:bg-indigo-50 hover:text-indigo-600 flex items-center gap-2.5 cursor-pointer transition-colors"
                                    >
                                      <Download className="w-3.5 h-3.5 text-slate-400" /> Download Manifest
                                    </button>
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation(); setOpenMenu(null);
                                        triggerToast(`Load ${load.id} put on hold.`, 'warning');
                                      }}
                                      className="w-full px-4 py-2 text-left text-[12px] font-semibold text-slate-700 hover:bg-amber-50 hover:text-amber-700 flex items-center gap-2.5 cursor-pointer transition-colors"
                                    >
                                      <Clock className="w-3.5 h-3.5 text-amber-500" /> Put On Hold
                                    </button>
                                  </div>
                                  ...
                                  <div className="py-1">
                                    <button
                                      onClick={async (e) => {
                                        e.stopPropagation(); setOpenMenu(null);
                                        if (window.confirm(`Cancel Load ${load.id}?`)) {
                                          try {
                                            await api.put(`/company-admin/loads/${load.rawId || load.id}`, { status: 'CANCELLED' });
                                          } catch (err) { /* ignore */ }
                                          setLoadsList(prev => prev.map(l => l.id === load.id ? { ...l, status: 'CANCELLED', statusSub: 'Cancelled' } : l));
                                          triggerToast(`Load ${load.id} cancelled.`, 'error');
                                        }
                                      }}
                                      className="w-full px-4 py-2 text-left text-[12px] font-semibold text-rose-600 hover:bg-rose-50 flex items-center gap-2.5 cursor-pointer transition-colors"
                                    >
                                      <X className="w-3.5 h-3.5 text-rose-500" /> Cancel Load
                                    </button>
                                  </div>
                                </div>
                              </>
