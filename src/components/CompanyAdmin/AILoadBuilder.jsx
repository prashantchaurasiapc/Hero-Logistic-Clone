...
export default function AILoadBuilder({ onBack }) {
  ...
  const [draft, setDraft] = useState({
    customer: selectedEmailData?.customer || 'Direct Customer',
    loadType: selectedEmailData?.loadType || 'General Freight',
  ...
  // Fetch real customers & loads from backend DB API
  useEffect(() => {
    ...
    api.get('/company-admin/loads')
      .then(res => {
        const loadsData = res.data?.data || res.data;
        if (Array.isArray(loadsData) && loadsData.length > 0) {
          const mappedBookings = loadsData.map(l => {
            const pickup = l.stops?.find(s => s.type === 'PICKUP')?.address || (l.stops && l.stops[0]?.address) || '100 Flinders St, Melbourne VIC 3000';
            const drop = l.stops?.find(s => s.type === 'DROPOFF')?.address || (l.stops && l.stops[l.stops.length - 1]?.address) || 'Nhava Sheva Port, Mumbai MH 400707';
            const customerName = l.customer?.name || (typeof l.customer === 'string' ? l.customer : 'Direct Customer');
            const loadRefStr = l.loadRef || l.id;

            return {
              id: l.id || loadRefStr,
              title: `Customer Booking Request – ${loadRefStr}`,
              sender: `Customer Portal · ${customerName}`,
              conf: l.priority === 'URGENT' || l.priority === 'HIGH' ? 'High' : 'Medium',
              isNew: l.status === 'DRAFT' || l.status === 'REQUESTED',
              ...
              items: Array.isArray(l.items) && l.items.length > 0
                ? l.items.map(it => ({ rego: it.rego || loadRefStr, vin: it.vin || it.stockRef || 'VIN-LIVE', make: it.make || 'General Cargo', model: it.model || 'Export Freight Box', year: '2024', colour: 'Blue', conf: 'High' }))
                : [{ rego: loadRefStr, vin: 'PO-LIVE', make: 'General Cargo', model: 'Export Freight Box', year: '2024', colour: 'Blue', conf: 'High' }],
              pricing: l.rate ? `$${parseFloat(l.rate).toLocaleString('en-US', { minimumFractionDigits: 2 })}` : '$3,450.00'
            };
          });

          setPortalBookings(mappedBookings);
          setEmails(mappedBookings.map(b => ({ ...b, title: `Email Booking – ${b.id}`, sender: `dispatch@${(b.customer || 'company').toLowerCase().replace(/[^a-z]/g, '')}.com` })));
          if (mappedBookings[0]) {
            setSelectedEmail(mappedBookings[0].id);
          }
        }
      })
      .catch(() => {});
  ...
  const handleCreateDraft = async () => {
    ...
    if (!draft.customer || !draft.customer.trim()) {
      setErrorMessage('Customer name is required before creating a draft load.');
      return;
    }
    if (!draft.pickupAddr || !draft.pickupAddr.trim()) {
      setErrorMessage('Pickup address is required.');
      return;
    }
    if (!draft.dropAddr || !draft.dropAddr.trim()) {
      setErrorMessage('Drop-off address is required.');
      return;
    }
    ...
    try {
      ...
      if (created) {
        setCreatedLoadId(created.id || created.loadRef);
      }
      setDraftCreated(true);
      setStep(4);
    } catch (err) {
      console.error('Error saving AI draft load:', err);
      const msg = err.response?.data?.error?.message || err.message || 'Failed to create draft load in database.';
      setErrorMessage(msg);
    } finally {
        ...
        <div className="flex-1 min-w-0 w-full">
          ...
          {step === 3 && (
            <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-7 shadow-xs space-y-7">
              ...
              {errorMessage && (
                <div className="p-4 bg-rose-50 border border-rose-200 rounded-xl flex items-center justify-between gap-3 text-xs font-bold text-rose-700">
                  <div className="flex items-center gap-2">
                    <AlertCircle size={16} className="text-rose-600 shrink-0" />
                    <span>{errorMessage}</span>
                  </div>
                  <button onClick={() => setErrorMessage(null)} className="text-rose-400 hover:text-rose-600 cursor-pointer"><X size={14} /></button>
                </div>
              )}
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {/* Customer Field with DB Datalist */}
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Customer / Owner</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Load Type</label>
                    <select
                      value={draft.loadType}
                      onChange={e => updateDraft('loadType', e.target.value)}
                      className={inpCls}
                    >
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Load Reference</label>
                    <input type="text" value={draft.loadRef} onChange={e => updateDraft('loadRef', e.target.value)} className={inpCls} />
                  </div>
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Estimated Pricing</label>
                    <input type="text" value={draft.pricing} onChange={e => updateDraft('pricing', e.target.value)} className={inpCls} />
                  </div>
                </div>
              </section>
