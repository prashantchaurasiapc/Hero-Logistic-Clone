...
export default function Messages() {
  ...
  const allContacts = [
    { id: 1, name: 'Dispatch Team', time: '09:15 AM', message: 'New load L-12548 assigned to you.', badge: 1, active: false },
    { id: 2, name: 'John D. (Driver)', time: '10:24 AM', message: 'At delivery location, waiting for g...', badge: 2, active: true },
    { id: 3, name: 'Mike T.', time: '08:47 AM', message: 'Can you confirm pickup time win...', badge: 3, active: false },
    { id: 4, name: 'Customer Support', time: 'Yesterday', message: 'Invoice INV-3021 has been gener...', badge: null, active: false }
  ];
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1400px] mx-auto bg-[#FAFAFA] min-h-screen text-left flex flex-col font-sans">
        ...
        <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-140px)] min-h-[600px]">
           ...
           {/* Sidebar (Contacts) */}
           <div className="w-full lg:w-[320px] flex-shrink-0 flex flex-col bg-transparent">
              ...
              <div className="flex flex-col gap-2 overflow-y-auto custom-scrollbar">
                  {filteredContacts.map(contact => (
                    <div 
                      key={contact.id} 
                      className={`flex flex-col p-4 rounded-xl cursor-pointer transition-colors border ${contact.active ? 'bg-white shadow-sm border-gray-100 relative' : 'bg-transparent hover:bg-white border-transparent hover:border-gray-100'}`}
                    >
                       {contact.active && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-[#FFD400] rounded-r-md"></div>}
                       <div className={`flex justify-between items-center mb-1 ${contact.active ? 'pl-2' : ''}`}>
                          <span className="text-[13px] font-bold text-gray-900">{contact.name}</span>
                          <span className={`text-[10px] ${contact.active ? 'font-bold text-gray-900' : 'font-semibold text-gray-400'}`}>{contact.time}</span>
                       </div>
                       <div className={`flex justify-between items-center ${contact.active ? 'pl-2' : ''}`}>
                          <span className={`text-[12px] font-medium ${contact.active ? 'text-gray-500' : 'text-gray-400'} truncate pr-4`}>{contact.message}</span>
                          {contact.badge && (
           ...
           {/* Main Chat Area */}
           <div className="flex-grow bg-white border border-gray-100 rounded-3xl flex flex-col shadow-sm overflow-hidden">
               ...
               <div className="px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-white z-10">
                  <div className="flex items-center gap-4">
                     ...
                     <div className="flex flex-col">
                        <span className="text-[14px] font-bold text-gray-900">John D. (Driver)</span>
               ...
               {/* Chat Messages */}
               <div className="flex-grow p-8 flex flex-col gap-6 overflow-y-auto custom-scrollbar bg-white">
                     ...
                     msg.sender === 'JD' ? (
                        <div key={msg.id} className="flex gap-4 max-w-[80%]">
                           ...
                           <div className="flex flex-col gap-1.5 items-start">
                              <div className="px-5 py-3.5 bg-white border border-gray-100 rounded-2xl rounded-tl-sm shadow-sm">
                                 <p className="text-[13px] font-medium text-gray-700">{msg.text}</p>
                              </div>
                        ...
                        <div key={msg.id} className="flex gap-4 max-w-[80%] self-end flex-row-reverse">
                           ...
                           <div className="flex flex-col gap-1.5 items-end">
                              <div className="px-5 py-3.5 bg-[#F8FAFC] border border-gray-100 rounded-2xl rounded-tr-sm shadow-sm">
                                 <p className="text-[13px] font-medium text-gray-700">{msg.text}</p>
                              </div>
               ...
               <div className="px-8 py-5 border-t border-gray-100 bg-white">
                  <div className="flex items-center gap-4">
                     ...
                     <input 
                        ...
                        className="flex-grow bg-white border border-gray-200 rounded-full px-5 py-3.5 text-[13px] font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#FFD400] placeholder-gray-400"
                     />
                     <button 
                        onClick={handleSend}
                        className="w-11 h-11 bg-[#FFD400] hover:bg-[#F0C800] rounded-full flex items-center justify-center text-black flex-shrink-0 shadow-sm transition-colors cursor-pointer"
                     >
