...
export default function FleetMonitor() {
  ...
  // On Road Summary Table Action States & Handlers
  const [openActionMenuId, setOpenActionMenuId] = useState(null);
  const [isContactDriverModalOpen, setIsContactDriverModalOpen] = useState(false);
  const [selectedContactDriver, setSelectedContactDriver] = useState(null);
  const [isQuickLoadModalOpen, setIsQuickLoadModalOpen] = useState(false);
  const [selectedQuickLoad, setSelectedQuickLoad] = useState(null);

  const handleCallDriver = (row) => {
    const targetDriver = driversList.find(d => d.name === row.driver || d.id === row.driverId) || {
      id: row.driverId || 'DRV-101',
      name: row.driver,
      avatar: row.avatar,
      phone: row.phone || '0412 345 678',
      status: row.status,
      statusStyle: row.statusStyle,
      loadId: row.loadId,
      vehicle: row.vehicle,
      routeFrom: row.route ? row.route.split(' → ')[0] : 'Origin',
      routeTo: row.route ? row.route.split(' → ')[1] : 'Destination'
    };
    setSelectedContactDriver(targetDriver);
    setIsContactDriverModalOpen(true);
    setOpenActionMenuId(null);
  };

  const cleanDriverName = (rawName, fallbackIdx = 0) => {
    const fallbacks = ['Alex Wright', 'John Doe', 'Chris Lee', 'Michael Tan', 'Daniel Craig', 'David Brown'];
    if (!rawName || typeof rawName !== 'string') return fallbacks[fallbackIdx % fallbacks.length];
    const cleaned = rawName.trim();
    if (cleaned.toLowerCase().includes('aasasd') || cleaned.toLowerCase().includes('unknown') || cleaned.length < 3) {
      return fallbacks[fallbackIdx % fallbacks.length];
    }
    return cleaned;
  };

  const handleFocusDriver = (row) => {
    const targetDriver = driversList.find(d => d.name === row.driver || d.id === row.driverId || d.loadId === row.loadId) || driversList[0];
    if (targetDriver) {
  ...
  const handleViewQuickLoad = (row) => {
    setSelectedQuickLoad(row);
    setIsQuickLoadModalOpen(true);
    setOpenActionMenuId(null);
  };

  const handleEyeViewClick = (row) => {
    handleFocusDriver(row);
    handleViewQuickLoad(row);
    triggerToast(`Viewing load details for ${row.loadId}`);
  };
  ...
  return (
    <div className="w-full min-h-screen bg-[#F8FAFC] p-4 sm:p-6 space-y-4 text-left font-sans antialiased text-slate-800">
                ...
                <tbody className="divide-y divide-slate-100 text-xs font-medium">
                  {summaryLoads.map((row) => (
                    <tr key={row.loadId} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-2 px-2 text-right whitespace-nowrap relative">
                        <div className="flex items-center justify-end gap-1 text-slate-500">
                          <button 
                            onClick={() => handleCallDriver(row)} 
                            title={`Call Driver ${row.driver}`}
                            className="p-1.5 hover:bg-blue-50 text-slate-600 hover:text-blue-600 rounded-md cursor-pointer transition-colors"
                          >
                            <Phone className="w-3.5 h-3.5" />
                          </button>

                          <button 
                            onClick={() => handleEyeViewClick(row)} 
                            title={`View Details & Focus Map for ${row.loadId}`}
                            className="p-1.5 hover:bg-blue-50 text-slate-600 hover:text-blue-600 rounded-md cursor-pointer transition-colors"
                          >
                          ...
                          <div className="relative inline-block text-left">
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                setOpenActionMenuId(openActionMenuId === row.loadId ? null : row.loadId);
                              }} 
                              title="More Actions"
                              className="p-1.5 hover:bg-slate-100 text-slate-600 rounded-md cursor-pointer transition-colors"
                            >
                              <MoreVertical className="w-3.5 h-3.5" />
                            </button>

                            {openActionMenuId === row.loadId && (
                              <>
                                <div 
                                  className="fixed inset-0 z-40" 
                                  onClick={() => setOpenActionMenuId(null)} 
                                />
                                <div className="absolute right-0 mt-1 w-48 bg-white border border-slate-200 rounded-xl shadow-xl z-50 p-1 space-y-0.5 text-left animate-in fade-in zoom-in-95">
                                  <button 
                                    onClick={() => handleFocusDriver(row)}
                                    className="w-full px-2.5 py-1.5 rounded-lg text-xs font-semibold text-slate-700 hover:bg-blue-50 hover:text-blue-600 flex items-center gap-2 cursor-pointer"
                                  >
                                    <Target className="w-3.5 h-3.5 text-blue-600" />
                                  ...
                                  <button 
                                    onClick={() => handleViewQuickLoad(row)}
                                    className="w-full px-2.5 py-1.5 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-100 flex items-center gap-2 cursor-pointer"
                                  >
                                  ...
                                  <button 
                                    onClick={() => {
                                      handleOpenSendLocationModal(row.driverId);
                                      setOpenActionMenuId(null);
                                    }}
                                    className="w-full px-2.5 py-1.5 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-100 flex items-center gap-2 cursor-pointer"
                                  >
                                    <Send className="w-3.5 h-3.5 text-emerald-600" />
                                    <span>Send Waypoint / Nav</span>
                                  </button>
                                  ...
                                  >
                                    ...
                                    <span>Call & Contact Driver</span>
                                  </button>

                                  <button 
                                    onClick={() => {
                                      triggerToast(`Flagged delay alert for ${row.loadId}`);
                                      setOpenActionMenuId(null);
                                    }}
                                    className="w-full px-2.5 py-1.5 rounded-lg text-xs font-semibold text-rose-600 hover:bg-rose-50 flex items-center gap-2 cursor-pointer border-t border-slate-100 mt-1 pt-1.5"
                                  >
                                    <AlertTriangle className="w-3.5 h-3.5 text-rose-600" />
                                    <span>Flag Delay Alert</span>
                                  </button>
                                </div>
                              </>
                            )}
      ...
      {isSendLocationModalOpen && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-[99999] flex items-center justify-center p-3 overflow-hidden">
      ...
      {isContactDriverModalOpen && selectedContactDriver && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-[99999] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-200/90 shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 text-left">
            ...
            <div className="p-4 bg-slate-900 text-white flex justify-between items-center border-b border-slate-800">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-bold tracking-tight">Contact Driver Directly</h3>
                  <p className="text-[10px] text-slate-400 font-medium">Direct line dispatch communication</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-5 space-y-4">
              <div className="flex items-center gap-3.5 p-3.5 bg-slate-50 border border-slate-200/80 rounded-xl">
                <div className="relative shrink-0">
                  <img 
                    src={selectedContactDriver.avatar} 
                    alt={selectedContactDriver.name} 
                    className="w-12 h-12 rounded-full object-cover border-2 border-white shadow-2xs" 
                  />
                  <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-emerald-500 rounded-full border-2 border-white" />
                </div>
                <div className="min-w-0 flex-1">
                  <h4 className="text-sm font-bold text-slate-900 leading-snug truncate">{selectedContactDriver.name}</h4>
                  <div className="text-xs font-semibold text-slate-600 flex items-center gap-1.5 mt-0.5">
                    <Phone className="w-3 h-3 text-slate-400 shrink-0" />
                    <span>{selectedContactDriver.phone || '0412 345 678'}</span>
                  </div>
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    <span className="px-2 py-0.5 bg-blue-100/80 text-blue-800 font-bold rounded text-[10px] border border-blue-200/60">
                      {selectedContactDriver.loadId}
                    </span>
                    <span className="px-2 py-0.5 bg-slate-200/60 text-slate-700 font-semibold rounded text-[10px]">
                      {selectedContactDriver.vehicle || 'Truck TR-01'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2.5 text-xs font-semibold">
                <a 
                  href={`tel:${selectedContactDriver.phone || '0412345678'}`}
                  onClick={() => triggerToast(`Initiating call to ${selectedContactDriver.name}...`)}
                  className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white rounded-xl font-bold flex items-center justify-center gap-2 cursor-pointer shadow-xs transition-all hover:shadow-md"
                >
                  <Phone className="w-4 h-4" />
                  <span>Call Phone ({selectedContactDriver.phone || '0412 345 678'})</span>
                </a>

                <button 
                  onClick={() => {
                    triggerToast(`Opening direct dispatch chat with ${selectedContactDriver.name}...`);
                    setIsContactDriverModalOpen(false);
                  }}
                  className="w-full py-3 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white rounded-xl font-bold flex items-center justify-center gap-2 cursor-pointer shadow-xs transition-all hover:shadow-md"
                >
                  <MessageSquare className="w-4 h-4" />
                  <span>Open Live Dispatch Chat</span>
                </button>

                <button 
                  onClick={() => {
                    handleOpenSendLocationModal(selectedContactDriver.id);
                    setIsContactDriverModalOpen(false);
                  }}
                  className="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold flex items-center justify-center gap-2 cursor-pointer transition-colors border border-slate-200/80"
                >
                  <Send className="w-4 h-4 text-blue-600" />
                  <span>Send Location & Waypoint</span>
                </button>
              </div>
            </div>
            ...
            <div className="p-3.5 bg-slate-50 border-t border-slate-100 text-right">
              <button 
                onClick={() => setIsContactDriverModalOpen(false)}
                className="px-4 py-1.5 bg-white border border-slate-200 text-slate-700 font-bold rounded-lg text-xs hover:bg-slate-100 cursor-pointer shadow-2xs"
              >
                Close
              </button>
            </div>
      ...
      {/* Quick Load Details Modal */}
      {isQuickLoadModalOpen && selectedQuickLoad && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md z-[99999] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl border border-slate-200/90 shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 text-left">
            ...
            <div className="p-4 bg-slate-900 text-white flex justify-between items-center border-b border-slate-800">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-bold tracking-tight">Load Details: {selectedQuickLoad.loadId}</h3>
                  <p className="text-[10px] text-slate-400 font-medium">Real-time load & route overview</p>
                </div>
              </div>
              <button 
                onClick={() => setIsQuickLoadModalOpen(false)}
                className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <div className="p-5 space-y-4 text-xs">
              <div className="flex items-center justify-between p-3.5 bg-blue-50/70 border border-blue-200/80 rounded-xl">
                <div>
                  <span className="text-[10px] font-bold text-blue-600 uppercase tracking-wider block">ROUTE</span>
                  <span className="text-sm font-extrabold text-slate-900">{selectedQuickLoad.route}</span>
                </div>
                <span className={`px-2.5 py-1 rounded-md text-xs font-bold border ${selectedQuickLoad.statusStyle}`}>
                  {selectedQuickLoad.status}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-slate-700">
                <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    <img src={selectedQuickLoad.avatar} alt={selectedQuickLoad.driver} className="w-5 h-5 rounded-full object-cover" />
                    <span className="font-bold text-slate-900">{selectedQuickLoad.driver}</span>
                  </div>
                </div>

                <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                  <span className="text-[10px] text-slate-400 font-bold uppercase block">Vehicle / Fleet</span>
                  <span className="font-bold text-slate-900">{selectedQuickLoad.vehicle}</span>
                </div>

                <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                  <span className="text-[10px] text-slate-400 font-bold uppercase block">ETA Next Stop</span>
                  <span className="font-bold text-slate-900">{selectedQuickLoad.etaNext}</span>
                </div>

                <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-xl space-y-1">
                  <span className="text-[10px] text-slate-400 font-bold uppercase block">ETA Final Delivery</span>
                  <span className="font-bold text-slate-900">{selectedQuickLoad.etaDelivery}</span>
                </div>
              </div>

              <div className="p-3.5 bg-emerald-50 border border-emerald-200/80 rounded-xl flex justify-between items-center">
                <span className="font-bold text-emerald-900">Cargo Progress Step</span>
                <span className="font-extrabold text-emerald-700 text-sm">{selectedQuickLoad.progressStep}</span>
              </div>
            </div>
            ...
            <div className="p-3.5 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
              <button 
                onClick={() => {
                  handleFocusDriver(selectedQuickLoad);
                  setIsQuickLoadModalOpen(false);
                }}
                className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg text-xs flex items-center gap-1.5 cursor-pointer shadow-2xs"
              >
