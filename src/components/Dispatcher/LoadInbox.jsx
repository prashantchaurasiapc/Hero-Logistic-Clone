  ...
  const fetchDraftLoads = async () => {
    try {
      ...
      const formatted = dbLoads.map((load, i) => {
        ...
        const sourceRaw = (load.sourceType || load.source || '').toLowerCase();
        const source = sourceRaw.includes('email') ? 'email' : sourceRaw.includes('file') || sourceRaw.includes('pdf') ? 'file' : 'portal';
        const driver = load.driver || {};
        const vehicle = load.truck || {};
        const pickupStop = load.pickupStop || load.stops?.find(s => s.type === 'PICKUP') || load.stops?.[0];
        const dropoffStop = load.dropoffStop || load.stops?.find(s => s.type === 'DROPOFF') || load.stops?.[1];
        const itemsList = load.items || load.loadItems || [];

        const formatStopDate = (stop) => {
          if (!stop) return 'TBD';
          const t = stop.scheduledDate || stop.scheduledTime;
          if (!t) return 'TBD';
          const d = new Date(t);
          return isNaN(d.getTime()) ? 'TBD' : d.toLocaleDateString('en-GB');
        };
        ...
        return {
          ...
          time: load.createdAt ? new Date(load.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Recently',
          urgent: load.priority === 'HIGH' || load.priority === 'URGENT',
          confidence: 'High',
          driver: driver.name || (driver.firstName ? `${driver.firstName} ${driver.lastName}` : 'Unassigned'),
          avatar: driver.name ? driver.name.slice(0, 2).toUpperCase() : (driver.firstName ? `${driver.firstName[0]}${driver.lastName?.[0] || ''}` : 'U'),
          avatarColor: '#2563eb',
          driverPhone: driver.phone || driver.contactNumber || 'N/A',
          driverLicence: driver.licenseType || 'N/A',
          vehicle: vehicle.rego ? `${vehicle.rego} · ${vehicle.make}` : 'Unassigned',
          trailer: load.trailer?.rego ? `${load.trailer.rego}` : 'Trailer TBD',
          volume: `${itemsList.length} Vehicle${itemsList.length === 1 ? '' : 's'}`,
          from: pickupStop?.address || load.pickupLocation || 'Pickup TBD',
          to: dropoffStop?.address || load.deliveryLocation || 'Dropoff TBD',
          pickupDate: formatStopDate(pickupStop),
          deliveryDate: formatStopDate(dropoffStop),
          accentColor: '#4f46e5',
          notes: load.notes || 'No notes provided.',
          manifests: itemsList.map((item, idx) => ({
            rego: item.rego || item.stockRef || `REG-${100 + idx}`,
            vin: item.vin || item.stockRef || 'N/A',
            model: `${item.make || ''} ${item.model || ''}`.trim() || item.description || 'Cargo Asset',
            colour: item.color || item.colour || 'Standard',
            conf: 'High'
          }))
        };
      });
