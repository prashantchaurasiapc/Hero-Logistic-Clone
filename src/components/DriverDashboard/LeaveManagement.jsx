  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-gray-50 min-h-screen text-left flex flex-col space-y-6 relative pb-28">
      ...
      <div className="w-full space-y-6">
        ...
        <div className="bg-white border border-gray-150 rounded-2xl sm:rounded-3xl p-4 sm:p-6 flex justify-between items-center shadow-sm gap-4">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-lg sm:text-2xl font-black text-[#0F172A]">leave management</span>
            </div>
        ...
        {/* Top Banner */}
        <div className="bg-white border border-[#FEF08A] rounded-2xl sm:rounded-[2rem] p-4 sm:p-6 shadow-sm">
          ...
          <p className="text-xs sm:text-sm font-medium text-[#64748B]">Request vacation or medical rest logs.</p>
        </div>
        ...
        <div className="flex flex-col space-y-4">
          ...
          {selectedRows.length > 0 && (
            <div className="bg-[#FFFBEB] border border-[#FEF08A] rounded-2xl p-2 px-4 flex items-center justify-between w-full sm:w-max gap-4 shadow-sm">
                ...
                >
                  <div className="flex justify-between items-start gap-2">
                    <div className="flex items-center gap-3">
                      ...
                      {visibleColumns.type && (
                        <div>
                          <span className={`${labelSize} font-black text-gray-400 uppercase tracking-widest block`}>Leave Type</span>
                  ...
                  {visibleColumns.dates && (
                    <div className="pt-3 border-t border-gray-100">
                      <span className={`${labelSize} font-black text-gray-400 uppercase tracking-widest block mb-0.5`}>Requested Dates</span>
                      <span className={`font-bold text-[#334155] ${textSize} whitespace-pre-line`}>
                        {row.dates.replace(/\n/g, ' ')}
                      </span>
                    </div>
                  )}
              ...
              <tbody>
                {mockData.map((row, index) => {
                  ...
                  <tr key={index} className={`border-b border-gray-50 hover:bg-gray-50/50 transition-colors ${
                    ...
                    {visibleColumns.dates && (
                      <td className={`p-4 font-black text-[#334155] align-middle whitespace-nowrap ${viewMode === 'COMPACT' ? 'py-2' : viewMode === 'RELAXED' ? 'py-8' : 'py-6'}`}>
                        {row.dates.replace(/\n/g, ' ')}
                      </td>
                    )}
                    {visibleColumns.status && (
