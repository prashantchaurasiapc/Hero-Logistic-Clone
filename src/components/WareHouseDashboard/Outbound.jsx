...
export default function Outbound({ outboundData, setOutboundData, onScanOutClick, onPrintLabelClick }) {
        ...
        <table className="min-w-full divide-y divide-slate-200 text-left text-xs">
          <thead className="bg-slate-50 text-[10px] font-bold text-slate-400 uppercase tracking-wide">
            <tr>
              ...
              <th className="px-6 py-4 font-extrabold whitespace-nowrap">Outbound ID</th>
              <th className="px-6 py-4 font-extrabold whitespace-nowrap">Transport Carrier</th>
              <th className="px-6 py-4 font-extrabold whitespace-nowrap">Outbound Cargo Specs</th>
              <th className="px-6 py-4 font-extrabold whitespace-nowrap">Dock Gate Lane</th>
              <th className="px-6 py-4 font-extrabold whitespace-nowrap">Loading Actions</th>
            </tr>
          </thead>
            ...
            {outboundData.map((row) => {
              ...
              return (
                <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className={`text-slate-655 font-bold whitespace-nowrap ${getPaddingClass()}`}>
                    {row.carrier}
                  </td>
                  <td className={`text-slate-505 whitespace-nowrap ${getPaddingClass()}`}>
                    {row.cargo}
                  </td>
                  <td className={`text-[#f59e0b] font-bold font-mono whitespace-nowrap ${getPaddingClass()}`}>
                    {row.lane}
                  </td>
                  <td className={`whitespace-nowrap ${getPaddingClass()}`}>
                    <div className="flex gap-2">
                      <button
                        onClick={() => onScanOutClick(row)}
