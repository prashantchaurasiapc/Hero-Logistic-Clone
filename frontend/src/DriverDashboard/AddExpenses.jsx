...
export default function AddExpenses() {
  ...
  const [category, setCategory] = useState('Highway Toll pass');
  const [receiptUploaded, setReceiptUploaded] = useState(true);
  
  // AI Receipt Reader states
  ...
  // Logged expenses list state
  const [expenses, setExpenses] = useState([
    { id: 1, category: 'Fuel', amount: '$320.00', status: 'APPROVED' },
    { id: 2, category: 'Tolls', amount: '$42.50', status: 'PENDING' }
  ]);
  ...
  const handleAddExpense = (e) => {
    e.preventDefault();
    if (!amount.trim()) {
      triggerToast('Error: Please enter an expense amount.');
      return;
    }
    ...
    triggerToast(`Expense for ${category} (${formattedAmount}) logged successfully.`);
    setAmount('');
  ...
  const handleUploadReceipt = () => {
    ...
    triggerToast('Receipt file uploaded. AI Receipt reader is analyzing...');
    setAiDetected(true);
  };
  ...
  const handleConfirmAI = () => {
    ...
    setExpenses([newRecord, ...expenses]);
    setAiDetected(false);
    triggerToast('AI extracted expense of $420.50 (Fuel) confirmed and logged.');
    if (isOffline) {
      setOfflineQueueCount(prev => prev + 1);
    }
  };
  ...
  const handleRejectAI = () => {
    setAiDetected(false);
    triggerToast('AI parsed receipt data rejected.');
  };

  const handleReRunAI = () => {
    setAiLoading(true);
    triggerToast('Rerunning AI Receipt Reader extraction model...');
    setTimeout(() => {
      setAiLoading(false);
      triggerToast('AI analysis completed.');
    }, 1500);
  };
  ...
  const handleToggleConnection = () => {
    if (isOffline) {
      ...
      if (offlineQueueCount > 0) {
        triggerToast(`Synced ${offlineQueueCount} queued expenses online.`);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-[800px] mx-auto bg-gray-55 min-h-screen text-left flex flex-col space-y-6 relative pb-28">
      ...
      {/* Log Trip Expense Card */}
      <div className="bg-white border border-gray-150 rounded-3xl p-6 shadow-3xs text-left space-y-5">
        <span className="text-[10px] font-black text-gray-900 tracking-wider block uppercase">Log Trip Expense</span>

        <form onSubmit={handleAddExpense} className="space-y-4">
          <div>
            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">EXPENSE AMOUNT SPENT (USD)</label>
            <input 
              type="number"
              placeholder="e.g. 150"
              value={amount}
          ...
          <div>
            ...
            >
              <option value="Highway Toll pass">Highway Toll pass</option>
              <option value="Fuel">Fuel</option>
              <option value="Maintenance / Repairs">Maintenance / Repairs</option>
              <option value="Meals / Lodging">Meals / Lodging</option>
            </select>
          </div>

          {/* Receipt Upload area */}
          <div className="border border-dashed border-gray-255 rounded-2xl p-6 text-center hover:bg-gray-50/50 cursor-pointer select-none transition-colors">
            <Upload className="w-5 h-5 text-gray-400 mx-auto mb-1.5" />
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
              {receiptUploaded ? 'Receipt Uploaded ✓' : 'Click to Upload Receipt'}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <button 
              ...
              className="px-4 py-3 border border-amber-250 hover:bg-amber-50 text-[#D97706] rounded-xl text-xs font-bold transition-all cursor-pointer bg-white text-center uppercase"
            >
              Upload Receipt
            </button>
          </div>
        ...
        {/* AI Receipt Reader panel Card */}
        {aiDetected && (
          <div className="border border-amber-200 bg-amber-50/20 p-4.5 rounded-2xl text-left space-y-3.5 relative overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            {aiLoading ? (
              <div className="py-6 text-center text-xs font-bold text-gray-400 animate-pulse">Running AI extraction...</div>
            ) : (
              <>
                <span className="text-[9px] font-black text-amber-600 tracking-wider block uppercase">AI RECEIPT READER EXTRACT</span>
                <div>
                  <h4 className="text-xs font-bold text-gray-900">Expense detected: $420.50</h4>
                  <span className="text-[9px] text-gray-450 font-bold mt-0.5 block">Source: Pilot Travel Center</span>
                </div>
                <div className="flex gap-2">
                  <button 
                    onClick={handleReRunAI}
                    className="py-1 px-3 border border-gray-250 text-gray-700 font-bold text-[9px] rounded-lg cursor-pointer hover:bg-gray-50 uppercase bg-white shrink-0"
                  >
                    RE-RUN AI RECEIPT
                  </button>
                  <button 
                  ...
                  <button 
                    ...
                    className="py-1 px-3 border border-red-200 text-red-500 font-bold text-[9px] rounded-lg cursor-pointer hover:bg-red-50 uppercase shrink-0 ml-auto"
                  >
                    Reject
                  </button>
                </div>
      ...
      {/* Logged Expenses History card */}
      <div className="bg-white border border-gray-150 rounded-3xl p-6 shadow-3xs text-left space-y-5">
        <div className="flex justify-between items-center pb-2 border-b border-gray-55 flex-wrap gap-3">
          <span className="text-[10px] font-black text-gray-900 tracking-wider block uppercase">LOGGED EXPENSES HISTORY</span>
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              onClick={() => triggerToast('Column configurations opened.')}
              className="p-1.5 border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-500 hover:text-black cursor-pointer shadow-3xs bg-white shrink-0"
              title="Columns layout settings"
            >
        ...
        {/* Dynamic spacing items stack matching table data view */}
        <div className="space-y-3">
          <div className="grid grid-cols-12 text-[10px] font-bold text-gray-400 uppercase tracking-wider px-4">
            <span className="col-span-1"></span>
            <span className="col-span-5 text-left">CATEGORY</span>
            <span className="col-span-3 text-left">AMOUNT</span>
          ...
          <div className="divide-y divide-gray-55 border border-gray-150 rounded-2xl overflow-hidden bg-white shadow-3xs">
            {expenses.map((item) => (
              ...
              >
                ...
                <span className="col-span-5 text-xs font-bold text-gray-900 text-left">{item.category}</span>
                <span className="col-span-3 text-xs font-bold text-gray-755 text-left">{item.amount}</span>
