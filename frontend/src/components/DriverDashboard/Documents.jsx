...
export default function Documents() {
  ...
  const [vehicleData, setVehicleData] = useState({ truck: '', truckModel: '', trailer: '', trailerType: '' });
  ...
  const fetchDocuments = async () => {
    try {
      ...
      setVehicleDocs(data.vehicleDocs || []);
      setComplianceHistory(data.complianceHistory || []);
      if (data.vehicle) {
        setVehicleData(data.vehicle);
      }
      if (data.activeLoad) {
        setActiveLoadData(data.activeLoad);
      }
    } catch (err) {
      console.error('Failed to load driver documents:', err);
      triggerToast('Failed to load documents.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const compliancePercentage = (totalDocs - notRequiredCount) > 0 
    ? Math.round((validCount / (totalDocs - notRequiredCount)) * 100)
    : 100;

  // Filtering
  const filteredDocs = documents.filter(d => {
    const matchesSearch = d.name ? d.name.toLowerCase().includes(searchQuery.toLowerCase()) : true;
